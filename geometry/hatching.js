class Hatching {
  ...
  downwards() {
    const size = max(this.maxX - this.minX, this.maxY - this.minY);
  
    for (let i = -size; i < size * 2; i += this.spacing) {
      this.polylines.push(
        new Polyline([
          createVector(this.minX + i, this.minY),
          createVector(this.minX + i + size, this.minY + size)
        ])
      );
    }
  }
  ...
  upwards() {
    ...
    for (let i = -size; i < size * 2; i += this.spacing) {
      this.polylines.push(
        new Polyline([
          createVector(this.minX + i, this.maxY),
          createVector(this.minX + i + size, this.minY)
        ])
      );
    }
  }
